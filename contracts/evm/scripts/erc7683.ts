...
export const ACROSS_ORDER_DATA_TYPE_HASH = keccak256(toUtf8Bytes(ACROSS_ORDER_DATA_TYPE));
...
export function encodeAcrossOrderId(originData: string, destinationChainId: string) {
  const abiCoder = AbiCoder.defaultAbiCoder();
  return keccak256(abiCoder.encode(['bytes', 'uint256'], [originData, destinationChainId]));
}
